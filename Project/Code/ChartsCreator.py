...
def plot5Dchart(data_dir_path: str, countryName: str, dir_for_charts: str,
                new_csv_headers: Dict[int, str]):
    ...
    fig.suptitle(f'Stock Index change vs macroeconomic factors - {countryName}', fontsize=12)
    ...
    if min_interest_rate < 0:
        sizes = [float((data[new_csv_headers[4]][i]+abs(min_interest_rate)+0.01))*500
                 for i in range(len(data[new_csv_headers[4]]))]
    else:
        sizes = [float(data[new_csv_headers[4]][i])*500
                 for i in range(len(data[new_csv_headers[4]]))]
